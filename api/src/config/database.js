...
const gameDbPool = mysql.createPool({
  ...
  user: process.env.GAME_DB_USER || 'regnum_user',
  password: process.env.GAME_DB_PASS || 'regnum_pass',
...
const forumDbPool = mysql.createPool({
  ...
  user: process.env.COR_FORUM_DB_USER || 'corforum_user',
  password: process.env.COR_FORUM_DB_PASS || 'corforum_password',
  waitForConnections: true,
  connectionLimit: 5,
  queueLimit: 0
});
