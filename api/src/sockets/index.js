...
function initializeSocketHandlers(io) {
  ...
  io.on('connection', async (socket) => {
    ...
    // ==================== SHOUTBOX/CHAT OPERATIONS ====================
    
    // Initialize shoutbox handlers for this socket
    ...
    /* ================================================================
     *  3D FREE-LOOK MOVEMENT (server-authoritative with client prediction)
     *
     *  The client sends input vectors at a fixed tick rate.
     *  The server applies the same deterministic movement logic,
     *  clamps to world bounds, persists the position, broadcasts it,
     *  and sends the authoritative state back so the client can
     *  reconcile its prediction.
     * ================================================================ */

    // Per-socket 3D movement state
    const move3d = {
      active: false,
      lastTick: 0
    };
    const MOVE3D_SPEED = 20;      // world-units per server tick (matches pathfinding STEP_SIZE)
    const MOVE3D_SPRINT = 3;      // sprint multiplier
    const MOVE3D_MIN_TICK = 80;   // ms – minimum interval between accepted ticks (rate-limit)
    const WORLD_MIN = 0;
    const WORLD_MAX = 6144;

    /** Enter 3D mode */
    socket.on('move3d:enter', (data, callback) => {
      move3d.active = true;
      move3d.lastTick = Date.now();
      logger.info('Player entered 3D mode', { userId: user.userId });
      if (callback) callback({ success: true });
    });

    /** Exit 3D mode */
    socket.on('move3d:exit', (data, callback) => {
      move3d.active = false;
      logger.info('Player exited 3D mode', { userId: user.userId });
      if (callback) callback({ success: true });
    });

    /**
     * Receive movement input from the 3D client.
     * data: { seq, dx, dz, sprint, yaw }
     *   seq    – client sequence number for reconciliation
     *   dx, dz – normalised direction components (-1..1)
     *   sprint – boolean
     *   yaw    – camera yaw in radians (used to orient the direction)
     */
    socket.on('move3d:input', async (data) => {
      if (!move3d.active) return;

      const now = Date.now();
      if (now - move3d.lastTick < MOVE3D_MIN_TICK) return; // rate-limit
      move3d.lastTick = now;

      const { seq, dx, dz, sprint, yaw } = data || {};
      if (typeof dx !== 'number' || typeof dz !== 'number') return;
      ...
      if (len === 0) return; // no movement
      if (len > 1) { dirX /= len; dirZ /= len; }

      // Apply yaw rotation to get world-space direction
      const cosY = Math.cos(yaw);
      const sinY = Math.sin(yaw);
      const worldDx = dirX * cosY + dirZ * sinY;
      const worldDz = -dirX * sinY + dirZ * cosY;

      // Calculate displacement
      const speed = MOVE3D_SPEED * (sprint ? MOVE3D_SPRINT : 1);
      const moveX = worldDx * speed;
      const moveZ = worldDz * speed;
      ...
      try {
        ...
        await gameDb.query(
          'UPDATE players SET x = ?, y = ?, last_active = UNIX_TIMESTAMP() WHERE user_id = ?',
          [newX, newY, user.userId]
        );

        // Send authoritative state back to this client for reconciliation
        socket.emit('move3d:state', {
          seq,
          x: newX,
          y: newY
        });

        // Broadcast to all clients (for other-player visibility)
        io.emit('players:position', [{
          userId: user.userId,
          username: user.username,
          x: newX,
        ...
        try {
          const regions = require('../../gameData/regions.json');
          const prevRegionId = userRegions.get(user.userId) || null;
          const matched = regions.find(r => Array.isArray(r.coordinates) && pointInPolygon(newX, newY, r.coordinates));
          const newRegionId = matched ? (matched.id || null) : null;

          if (newRegionId !== prevRegionId) {
            userRegions.set(user.userId, newRegionId);
            if (prevRegionId) socket.emit('audio:stop', { type: 'music', regionId: prevRegionId });
            if (matched && matched.music) {
              try {
                const settings = socket.user && socket.user.settings ? socket.user.settings : null;
                if (settings && settings.musicEnabled) {
                  const vol = typeof settings.musicVolume === 'number' ? settings.musicVolume : parseFloat(settings.musicVolume) || 0.6;
                  socket.emit('audio:play', { type: 'music', file: matched.music, volume: vol, loop: true, regionId: newRegionId });
                }
              } catch (_) {}
            }
          }
        ...
        logger.error('move3d:input failed', { error: error.message, userId: user.userId });
      }
    });
