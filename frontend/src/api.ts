...
import { API_BASE, gameState } from './state';
 ...
 * @param endpoint - API path (appended to API_BASE)
...
export async function apiCall(endpoint: string, options: ApiCallOptions = {}): Promise<any> {
  ...
  if (!response.ok) {
    const err: ApiError = new Error((data && data.error) ? data.error : 'API call failed');
    err.status = status;
    err.body = data;
    throw err;
  }
 ...
 */
export async function emitOrApi(eventName: string, payload: any, fallbackPath: string, fallbackForm?: any): Promise<any> {
