...
import { isCasting, startCasting } from './castbar';
import { isSpellOnCooldown, getActiveSpellCount, getSpellCooldownRemaining } from './spells';
  ...
  try {
    ...
    if (invItem) {
      ...
      if (isSpell) {
        ...
        const { emitOrApi } = await import('./api');
