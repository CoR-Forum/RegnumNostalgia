...
import { gameState, batchUpdate } from './state';
import { apiCall } from './api';
import { getMap, getTotalH, getTotalW, gameToLatLng, getMapCenter, getDefaultZoom } from './map-state';
import { initializeWebSocket } from './socket-client';
import { showPlayerInfo, updatePlayerCoords, initHudButtons } from './player-ui';
import { createPlayerMarker } from './player';
import { enableClickToMove, buildGoHereIcon } from './walking';
import { initWindows, initWindow, saveWindowState, getWindowState } from './windows';
import { initInventoryDropZone } from './inventory';
import { initSpellsUI } from './spells';
...
async function initGame(progress) {
  try {
    ...
    await import('./map-init').then((mod) => mod.initMap());
